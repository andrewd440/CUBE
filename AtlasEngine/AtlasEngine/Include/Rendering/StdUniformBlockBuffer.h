...
public:
	...
	*/
	FStdUniformBlockBuffer(const char* BlockName, GLuint BindingIndexToSet, const uint32_t BlockSize);
	...
	* @param BlockSize - The size in bytes of the uniform block.
	*/
	FStdUniformBlockBuffer(const uint32_t BindingIndex, const uint32_t BlockSize);

	/**
	* Dtor
	* Deletes the buffer bound to this object.
	*/
	~FStdUniformBlockBuffer();
...
template <>
inline void FStdUniformBlockBuffer::SetData<FMatrix4>(const uint32_t DataOffset, const FMatrix4& Type)
{
	SetData(DataOffset, (uint8_t*)&Type.M[0][0], 64);
}

template <>
inline void FStdUniformBlockBuffer::SetData<FPerspectiveMatrix>(const uint32_t DataOffset, const FPerspectiveMatrix& Type)
{
	SetData(DataOffset, (uint8_t*)&Type.M[0][0], 64);
}

template <>
inline void FStdUniformBlockBuffer::SetData<LookAtMatrix>(const uint32_t DataOffset, const LookAtMatrix& Type)
{
	SetData(DataOffset, (uint8_t*)&Type.M[0][0], 64);
}

template <>
inline void FStdUniformBlockBuffer::SetData<Vector2f>(const uint32_t DataOffset, const Vector2f& Type)
{
	SetData(DataOffset, (uint8_t*)&Type.x, 8);
}

template <>
inline void FStdUniformBlockBuffer::SetData<Vector3f>(const uint32_t DataOffset, const Vector3f& Type)
{
	SetData(DataOffset, (uint8_t*)&Type.x, 12);
}

template <>
inline void FStdUniformBlockBuffer::SetData<Vector4f>(const uint32_t DataOffset, const Vector4f& Type)
{
	SetData(DataOffset, (uint8_t*)&Type.x, 16);
}

template <>
inline void FStdUniformBlockBuffer::SetData<FColor>(const uint32_t DataOffset, const FColor& Type)
{
